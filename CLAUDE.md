...
- **Mixed frequencies**: only the U.S. 10Y Treasury total-return index (Damodaran, 1928→) is still **annual** — no free monthly total-return source exists. The other six assets are **monthly**: gold (WGC/LBMA, 1978→), S&P 500 (Shiller/DataHub, 1871→), Nasdaq-100 (FRED, 1986→), and SSE Composite / CSI 300 / China Treasury Bond Index (Eastmoney kline, month-end close, 1990-12→ / 2005-01→ / 2003-02→). `data_loader` aligns a selection to the *coarsest* common frequency — any annual asset forces the whole set to annual (monthly series downsampled to each year's last month), `periods_per_year=1`; only an all-monthly selection stays monthly, `periods_per_year=12` (which `metrics` uses for annualization). Never upsample/interpolate annual into monthly. Rebalancing is now per-boundary: `portfolio.py` reads the price index and resets to target weights at real calendar boundaries — `monthly` every month, `quarterly` at month 3/6/9/12 ends, `annual` at December (`_rebalance_boundary_mask`); holdings drift between boundaries. So on an all-monthly selection the three modes diverge. On an annual (or mixed-frequency-downsampled-to-annual) index every row is the finest node, so all three reset every row and coincide — surface this in the UI when a selection is downsampled to annual.
- Filenames encode the range and frequency, e.g. `sp500_monthly_1871_2026.csv`, `csi300_monthly_2005_2026.csv`. Preserve this pattern when adding series.
