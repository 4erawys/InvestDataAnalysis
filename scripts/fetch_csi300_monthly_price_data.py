"""
Fetch monthly CSI 300 (沪深300) index close from Eastmoney kline API.

Source     : https://push2his.eastmoney.com/api/qt/stock/kline/get
             secid=1.000300, klt=103 (monthly), fqt=0 (unadjusted)
Coverage   : 2005-01 to present (last complete month)
Output     : data/processed/indices/csi300_monthly_2005_<endYear>.csv

Schema: year_month,index_level,frequency,source,source_url,notes
  - index_level = CSI 300 month-end last-trading-day close (unadjusted)
  - frequency   = monthly
"""
...
NOTES = (
    "CSI 300 (沪深300) monthly close, last trading day of each month; "
    "unadjusted (fqt=0); source: Eastmoney kline API secid=1.000300 klt=103"
)
...
def main() -> None:
    ...
    raw_path = RAW_DIR / "eastmoney_csi300_monthly.json"
    ...
    for line in klines:
        fields = line.split(",")
        date_str = fields[0]          # e.g. "2005-01-31"
    ...
    out_path = PROCESSED_DIR / f"csi300_monthly_{START_YEAR}_{end_year}.csv"
