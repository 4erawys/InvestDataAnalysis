"""
Fetch monthly SSE Composite (上证综合指数) close from Eastmoney kline API.

Source     : https://push2his.eastmoney.com/api/qt/stock/kline/get
             secid=1.000001, klt=103 (monthly), fqt=0 (unadjusted)
Coverage   : 1990-12 to present (last complete month; SSE opened Dec 1990)
Output     : data/processed/indices/sse_composite_monthly_1990_<endYear>.csv

Schema: year_month,index_level,frequency,source,source_url,notes
  - index_level = SSE Composite month-end last-trading-day close (unadjusted)
  - frequency   = monthly
"""
...
NOTES = (
    "SSE Composite (上证综合指数) monthly close, last trading day of each month; "
    "unadjusted (fqt=0); SSE opened Dec 1990 so data begins 1990-12; "
    "source: Eastmoney kline API secid=1.000001 klt=103"
)
...
def main() -> None:
    ...
    raw_path = RAW_DIR / "eastmoney_sse_composite_monthly.json"
    ...
    for line in klines:
        fields = line.split(",")
        date_str = fields[0]          # e.g. "1990-12-31"
    ...
    out_path = PROCESSED_DIR / f"sse_composite_monthly_{start_year}_{end_year}.csv"
